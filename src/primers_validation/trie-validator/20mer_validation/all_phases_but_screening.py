...
def save_valid_sofar(primer_set):
    with open('../../../../data/valid_primers_so_far.txt', 'w') as f:
...
def check_if_contains_fixed_length_complement_patterns_in_primers_set(primer,
    ...
    for i in range(PRIMER_BPS - MAX_INTER_COMP):
        rev_complement_pattern = rev_complement_primer[i:i + MAX_INTER_COMP + 1]
...
def process_file(file_path, primer_set, patterns_complement_of_max_inter_comp_len_set, trie, histogram,
    ...
    for index, primer in tqdm(enumerate(lines), total=total_primers, desc=f"Processing {file_path}"):
        if index % 20000 == 0:
            show_logs(index, count_primers_disqualify_by_inter_compliment,
                      histogram, primer_set, trie, count_primers_disqualify_by_tm,
                      count_primers_disqualify_by_secondary_structure,
                      count_primers_disqualify_by_hamming_distance)
        primer = primer.strip()
...
def main():
    ...
    try:
        file_path = os.path.join('../../../../data/output_20_sorted_V.txt')
